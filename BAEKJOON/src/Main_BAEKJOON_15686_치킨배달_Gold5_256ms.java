...
public class Main_BAEKJOON_15686_치킨배달_Gold5_256ms {
	static class Point {
		int r, c;
	...
	private static void demolishStore(int currentLocation, int cnt) {
		if(cnt == M) { // 현재 치킨집의 수와 치킨집의 최대 개수가 같으면 종료
			getAllDistance(); // 현재 상태에서 도시의 치킨 거리 구하기
			return;
		}
	...
	private static void getAllDistance() {
		...
		for(int i = 0; i < N; i++) {
			for(int j = 0; j < N; j++) {
				if(map[i][j] == '1') { // 현재 위치에 집이 있으면 거리 구하기
					int distance = Integer.MAX_VALUE;
					for(Point s : storeLocationList) { // 현재 있는 모든 치킨집을 순회하면서 가장 가까운 치킨거리를 업데이트하기
						distance = Math.min(Math.abs(s.r - i) + Math.abs(s.c - j), distance); 
					}
		...
		min = Math.min(min, allDistance); // 도시의 치킨 거리 업데이트
	}
